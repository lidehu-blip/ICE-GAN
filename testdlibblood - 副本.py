...
BUFFER_MAX_SIZE = 300  # Number of recent ROI average values to store
MAX_VALUES_TO_GRAPH = 40  # Number of recent ROI average values to show in the pulse graph
...
def run_pulse_observer(detector, predictor, webcam, window):
    ...
    while cv2.getWindowProperty(window, 0) == 0:
        ...
        h = size[0]
        print(h, w)
        frame = cv2.resize(frame, (int(w*0.5), int(h*0.5)), interpolation=cv2.INTER_CUBIC)
